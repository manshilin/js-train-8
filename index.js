// Завдання 1
// В даному завданні створимо функцію, яка використовує метод `Array.from` для створення масиву зі списку користувачів 
//з бази даних.
...
function getUsersFromDatabase(array) {
 let userArray = Array.from(array);
 userArray.forEach(element => {
    element.id = element.id;
    element.firstName = element.name.toUpperCase();
    element.years = element.age;
    delete element.name;
    delete element.age;
  });
  return userArray;
  ...
  // id який дорівнює id користувача,firstName який дорівнює firstName користувача в верхньому регістрі та years який 
  //дорівнює age користувача
...
function removeLastElement(arr) {
  ...
  let removeLastElement = arr.length - 1;
  arr.pop(removeLastElement);
  return arr;
...
function copyAndSwapElements(arr, target, start, end) {
  if (!Array.isArray(arr)) {
    return "Неприпустимий тип даних";
  }
